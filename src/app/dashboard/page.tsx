// src/app/dashboard/student/page.tsx
'use client';

import React, { useEffect, useState } from 'react';
import Grid from '@mui/material/Grid';
import {
  Card,
  CardContent,
  CardActions,
  Typography,
  Button,
  CircularProgress,
} from '@mui/material';
import SchoolIcon from '@mui/icons-material/School';
import { getCurrentUser } from '@aws-amplify/auth';
import { generateClient } from '@aws-amplify/api';
import type { GraphQLResult } from '@aws-amplify/api-graphql';

import {
  ListEnrollmentsQuery,
  GetCourseQuery,
  ListAnnouncementsQuery,
  ListAssignmentsQuery,
} from '@/graphql/API';
import {
  listEnrollments,
  getCourse as getCourseQuery,
  listAnnouncements,
  listAssignments,
} from '@/graphql/queries';

interface CourseWithAssignments {
  id: string;
  title?: string | null;
  description?: string | null;
  assignments: Array<{ id: string; title?: string | null; dueDate?: string | null }>;
}

export default function StudentDashboard() {
  const [loading, setLoading] = useState(true);
  const [courses, setCourses] = useState<CourseWithAssignments[]>([]);
  const [announcements, setAnnouncements] = useState<
    { id: string; title?: string | null; content?: string | null; createdAt: string }[]
  >([]);

  useEffect(() => {
    (async () => {
      try {
        const auth = await getCurrentUser();
        const username = auth.username;
        const client = generateClient();

        // 1️⃣ Fetch enrollments for this user
        const enrollResp = (await client.graphql({
          query: listEnrollments,
          variables: {
            filter: { userId: { eq: username } },
          },
        })) as GraphQLResult<ListEnrollmentsQuery>;

        const enrollmentItems = enrollResp.data?.listEnrollments?.items ?? [];
        const courseIds = enrollmentItems
          .map(e => e?.courseId)
          .filter((id): id is string => !!id);

        // 2️⃣ For each courseId, fetch the course and its assignments
        const coursePromises = courseIds.map(async courseId => {
          const courseResp = (await client.graphql({
            query: getCourseQuery,
            variables: { id: courseId },
          })) as GraphQLResult<GetCourseQuery>;

          const course = courseResp.data?.getCourse;
          if (!course) return null;

          // list assignments for this course
          const assignResp = (await client.graphql({
            query: listAssignments,
            variables: {
              filter: { courseId: { eq: courseId } },
            },
          })) as GraphQLResult<ListAssignmentsQuery>;

          const assignmentItems =
            assignResp.data?.listAssignments?.items.filter((a): a is any => !!a) ?? [];

          return {
            id: course.id,
            title: course.title,
            description: course.description,
            assignments: assignmentItems.map(a => ({
              id: a.id,
              title: a.title,
              dueDate: a.dueDate,
            })),
          } as CourseWithAssignments;
        });

        const loadedCourses = (await Promise.all(coursePromises)).filter(
          (c): c is CourseWithAssignments => c !== null
        );
        setCourses(loadedCourses);

        // 3️⃣ Fetch recent announcements (optional)
        const annResp = (await client.graphql({
          query: listAnnouncements,
          variables: { limit: 5 },
        })) as GraphQLResult<ListAnnouncementsQuery>;
        const annItems =
          annResp.data?.listAnnouncements?.items
            .filter((a): a is any => !!a)
            .map(a => ({
              id: a.id,
              title: a.title,
              content: a.content,
              createdAt: a.createdAt,
            })) ?? [];
        setAnnouncements(annItems);
      } catch (err) {
        console.error('StudentDashboard error:', err);
      } finally {
        setLoading(false);
      }
    })();
  }, []);

  if (loading) {
    return <CircularProgress />;
  }

  return (
    <div style={{ padding: 24 }}>
      <Typography variant="h4" gutterBottom>
        <SchoolIcon style={{ verticalAlign: 'middle' }} /> Your Courses
      </Typography>

      <Grid container spacing={3}>
        {courses.map(course => (
          <Grid item xs={12} sm={6} md={4} key={course.id}>
            <Card elevation={3}>
              <CardContent>
                <Typography variant="h6">{course.title}</Typography>
                <Typography color="textSecondary">{course.description}</Typography>
                <Typography variant="body2" sx={{ mt: 1, fontWeight: 'bold' }}>
                  Assignments:
                </Typography>
                {course.assignments.length ? (
                  course.assignments.map(a => (
                    <Typography key={a.id} variant="body2">
                      • {a.title} (Due {a.dueDate ?? '—'})
                    </Typography>
                  ))
                ) : (
                  <Typography variant="body2">No assignments yet.</Typography>
                )}
              </CardContent>
              <CardActions>
                <Button size="small" variant="contained">
                  View Course
                </Button>
              </CardActions>
            </Card>
          </Grid>
        ))}
      </Grid>

      {announcements.length > 0 && (
        <>
          <Typography variant="h5" sx={{ mt: 4, mb: 2 }}>
            Latest Announcements
          </Typography>
          {announcements.map(a => (
            <Card key={a.id} sx={{ mb: 2 }} elevation={2}>
              <CardContent>
                <Typography variant="h6">{a.title}</Typography>
                <Typography variant="body2" color="textSecondary">
                  {new Date(a.createdAt).toLocaleDateString()}
                </Typography>
                <Typography sx={{ mt: 1 }}>{a.content}</Typography>
              </CardContent>
            </Card>
          ))}
        </>
      )}
    </div>
  );
}